...
float sdVoxels(vec3 p) {
  float minDist = 100.0;
  float gridSize = 1.0;

  vec2 pVoxelCenter = floor(p.xy * gridSize) / gridSize;

  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      vec3 voxelPos = vec3(
        pVoxelCenter.x + float(i - 1) / gridSize,
        pVoxelCenter.y + float(j - 1) / gridSize,
        0.5
      );

      float rotationX = random(voxelPos.xy);
      float rotationY = random(voxelPos.yx * 0.2);
      float rotationZ = random(voxelPos.yx * 0.3);

      vec3 voxelP = p;

      voxelPos.z -= (sin(rotationZ * 4.0 + uTime) + 1.0) * 1.0;

      voxelP = Translate(voxelP, voxelPos);
      voxelP = Rotate(voxelP, vec3(0.0, rotationY, rotationX));

      // float piramidScale = 0.3;
      // float voxel = sdPyramid(voxelP / piramidScale, 1.0) * piramidScale;

      float voxel = sdBox(voxelP, vec3(0.2));
      minDist = min(minDist, voxel);
    }
  }
...
float getSceneHit(vec3 p) {
  vec3 floorP = Translate(p, vec3(0.0, 0.0, getNoiseLevel(p.xy) * 0.3));
  float floorPlane = sdPlane(floorP, vec4(0.0, 0.0, 1.0, 0.0));
