...
#pragma glslify: fxaa = require('./antialias.glsl', texture=texture, resolution=resolution, v_rgbNW=v_rgbNW, v_rgbNE=v_rgbNE, v_rgbSW=v_rgbSW, v_rgbSE=v_rgbSE, v_rgbM=v_rgbM)
...
void main() {
  ...
  vec4 insideSample = texture(uInsideColorTexture, vUv);
  vec4 raymarchSample = texture(uRaymarchTexture, vUv);
  vec4 outsideSample = texture(uOutsideColorTexture, vUv);

  vec3 color = backgroundColor;

  color += insideSample.rgb * insideSample.a;
  color = mix(color, raymarchSample.rgb, raymarchSample.a);
  color += outsideSample.rgb * outsideSample.a;
