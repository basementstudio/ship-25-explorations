...
class FlipFluid {
  ...
  constructor(
    ...
    this.particlePos = new Float32Array(3 * this.maxParticles)
  ...
  integrateParticles(dt: number, gravity: number, atractor?: Atractor): void {
    for (let i = 0; i < this.numParticles; i++) {
      ...
      if (atractor) {
        ...
        const dx = this.particlePos[3 * i] - atractor.x
        const dy = this.particlePos[3 * i + 1] - atractor.y
  ...
  pushParticlesApart(numIters: number): void {
    ...
    for (let i = 0; i < this.numParticles; i++) {
      const x = this.particlePos[3 * i]
      const y = this.particlePos[3 * i + 1]
    ...
    for (let iter = 0; iter < numIters; iter++) {
      for (let i = 0; i < this.numParticles; i++) {
        const px = this.particlePos[3 * i]
        const py = this.particlePos[3 * i + 1]
        ...
        for (let xi = x0; xi <= x1; xi++) {
          for (let yi = y0; yi <= y1; yi++) {
            ...
            for (let j = first; j < last; j++) {
              ...
              if (id === i) continue
              const qx = this.particlePos[3 * id]
              const qy = this.particlePos[3 * id + 1]
              ...
              dx *= s
              dy *= s
              this.particlePos[3 * i] -= dx
              this.particlePos[3 * i + 1] -= dy
              //this.particlePos[3 * i + 2] = 0
              this.particlePos[3 * id] += dx
              this.particlePos[3 * id + 1] += dy
              //this.particlePos[3 * id + 2] = 0
  ...
  handleParticleCollisions(
    ...
    for (let i = 0; i < this.numParticles; i++) {
      let x = this.particlePos[3 * i]
      let y = this.particlePos[3 * i + 1]
      ...
      this.particlePos[3 * i] = x
      this.particlePos[3 * i + 1] = y
      //this.particlePos[3 * i + 2] = 0
    }
  }
...
export function setupScene({ isDarkMode }: SetupSceneOptions): typeof scene {
  ...
  const relWaterHeight = 0.3
  const relWaterWidth = 0.3
