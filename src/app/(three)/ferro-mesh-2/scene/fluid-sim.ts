...
class FlipFluid {
  ...
  private remapedPointAPos: [number, number, number] | null = null
  ...
  calculateSmoothParticles(x: number, y: number, i: number) {
    ...
    if (this.pointAPos) {
      let zOffset = Math.abs(currentPos[X])
      // zOffset *= 1
      zOffset = Math.pow(zOffset * 2, 2) / 2
      this.remapedPointAPos = [
        this.pointAPos[X],
        this.pointAPos[Y],
        this.pointAPos[Z] - zOffset
      ]
      ...
      // get noise based on position & time
      let pointANoise = noise3D(
        this.particlePosLerp[particleIndex] * 10,
        this.elapsedTime * 0.05,
        this.particlePosLerp[particleIndex + 1] * 10 + this.elapsedTime * 0.1
      )
